...
impl NetworkBehaviourEventProcess<ping::PingEvent> for NyumtClientProt {
    fn inject_event(&mut self, event: ping::PingEvent) {
    }
}

#[async_trait]
impl NetworkBehaviourEventProcess<RequestResponseEvent<NyumtReq, NyumtResp>> for NyumtClientProt {
        ...
        match message {
            RequestResponseEvent::Message { peer, message: RequestResponseMessage::Request { request, channel, ..} } => {
                ...
                tokio::task::spawn_blocking(move || {
                    futures::executor::block_on(req_resp::handle(&request.0, peer, sett, channel));
                });
            },
            RequestResponseEvent::Message { peer, message: RequestResponseMessage::Response { response, request_id } } => {
                let sett = self.sett.clone();
                tokio::task::spawn_blocking(move || {
                    futures::executor::block_on(req_resp::handle_resp(&response.0, peer, sett, request_id));
                });
            },
            _ => ()
        }
