...
use libc::{c_char, c_void, c_int, size_t, sysctl, sysctlnametomib, timeval, statfs, ifaddrs, getifaddrs, if_data, freeifaddrs};
use data::*;
use super::common::*;
use super::unix;
use super::bsd;
...
lazy_static! {
    ...
    static ref PHYSMEM: [c_int; 4]       = sysctl_mib!(4, "hw.physmem");
    ...
    static ref HW_DISKNAMES: [c_int; 2] = sysctl_mib!(2, "kern.disks");
...
impl Platform for PlatformImpl {
    ...
    /*fn disk_names(&self) -> io::Result<Vec<String>> {
        let mut drives: *mut c_char = std::ptr::null_mut();
        let mut size = 0;
        // first time we just check size
        if unsafe { sysctl(&HW_DISKNAMES[0], HW_DISKNAMES.len() as u32, ptr::null_mut(), &mut size, ptr::null_mut(), 0) } != 0 {
                return Err(io::Error::new(io::ErrorKind::Other, "sysctl() failed"));
        }
        drives = unsafe { libc::malloc(size) } as *mut i8;
        sysctl!(HW_DISKNAMES, drives, size);
        if drives.is_null() {
            return Err(io::Error::new(io::ErrorKind::Other, "sysctl() failed"));
        }
        let c_str: &std::ffi::CStr = unsafe { std::ffi::CStr::from_ptr(drives) };
        let str_disks: String = match c_str.to_str() {
            Ok(v)  => v.to_owned(),
            Err(_) => return Err(io::Error::new(io::ErrorKind::Other, "C string cannot be converted"))
        };
        unsafe { libc::free(drives as *mut _ as *mut c_void) };
        let mut disks: Vec<String> = Vec::new();
        for disk in str_disks.split(" ") {
