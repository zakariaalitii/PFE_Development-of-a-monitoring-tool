...
use std::mem::size_of;
use libc::{c_char, c_void, c_int, c_uint, c_ulong, c_uchar, ioctl, sysctl, timeval, ifaddrs, getifaddrs, if_data, freeifaddrs, statfs};
use data::*;
use super::common::*;
use super::unix;
use super::bsd;
...
lazy_static! {
    ...
    static ref HW_DISKNAMES: [c_int; 2] = [6, 8];
...
impl Platform for PlatformImpl {
    ...
    /*fn disk_names(&self) -> io::Result<Vec<String>> {
        ...
        for disk in str_disks.split(",") {
            if !disk.ends_with(":") {
                disks.push(match disk.split(":").next() {
                    Some(v) => v,
                    None    => continue
                }.to_owned());
            }
